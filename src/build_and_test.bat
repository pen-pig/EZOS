...
if errorlevel 1 goto error
rem "%CC%" %CFLAGS% -c kernel\gfx.c -o kernel\gfx.o  &rem gfx 图形模式已注释
if errorlevel 1 goto error

echo [4/6] 链接内核...
"%LD%" %LDFLAGS% -o kernel_raw.bin boot\kernel_entry.o kernel\kernel.o kernel\tty.o kernel\idt.o kernel\isr.o kernel\keyboard.o kernel\ata.o kernel\shell.o kernel\exfat.o kernel\gui.o kernel\mouse.o
if errorlevel 1 goto error

echo [5/6] 填充内核到 64KB...
"%OBJCOPY%" -I binary -O binary --pad-to 65536 kernel_raw.bin kernel.bin
if errorlevel 1 goto error
...
:run
echo 正在启动 QEMU...
"%QEMU%" -drive format=raw,file=os-image.bin -drive format=raw,file=disk.img
pause
goto end

:clean
echo 正在清理构建产物...
del /q boot\boot.bin boot\kernel_entry.o kernel\kernel.o kernel\tty.o kernel\idt.o kernel\isr.o kernel\keyboard.o kernel\ata.o kernel\shell.o kernel\exfat.o kernel\gui.o kernel\mouse.o kernel_raw.bin kernel.bin os-image.bin 2>nul
echo 清理完成！
pause
exit /b 0
