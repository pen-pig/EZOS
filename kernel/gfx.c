...
void gfx_restore_text(void) {
    ...
    outb(0x3D4, 0x0C); outb(0x3D5, 0xC0);   /* start_addr ��λ = 0xC0 */
    outb(0x3D4, 0x0D); outb(0x3D5, 0x00);   /* start_addr = 0xC000������ B8000 �� */
    outb(0x3D4, 0x0E); outb(0x3D5, 0x00);
    outb(0x3D4, 0x0F); outb(0x3D5, 0x00);
...
#if 1 // EXPERIMENT5: д 0x400..0x40F=0xFF������ GC4=2��read plane 2������ QMP pmemsave ����
    ...
    outb(0x3CE, 0x06); outb(0x3CF, 0x04);   /* GC6 graphics plane mode */
    {
        ...
        int fj;
        for (fj = 0; fj < 16; fj++)
            fp[0x400 + fj] = 0xFF;          /* 0x20 ���� 16 �� = ȫ 1 */
    }
    outb(0x3CE, 0x04); outb(0x3CF, 0x02);   /* read map: plane 2�����֣��� QMP ���أ� */
#endif
    outb(0x3CE, 0x04); outb(0x3CF, 0x02);   /* Read Map Select: plane 2��EXPERIMENT5 �����ã� */
