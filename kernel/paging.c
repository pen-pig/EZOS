 ...
 *
 * 关键约束：
 *  - 页结构必须放在"保证空闲"的物理内存：低 1MB 已被镜像（0x10000-0x8C000）
 *    与栈（自 0x90000 向下）、VGA（0xA0000+）占满；.bss.hi 1-2MB 是堆/dmesg/
 *    FS 缓冲，仅剩几十 KB；.bss 已解耦到 19MB（linker.ld）；2-4MB 虚拟段
 *    是 exec 装载的 PD[1] 暂存区，绝不能放常驻数据。
